class Stroll {
  ...
  async focus(name, push = true, replace = false) {
    ...
    this.view.title.innerHTML = `${page.hname}${
      this.options.titleBase ? " | " + this.options.titleBase : ""
    }`;
  ...
  async load(name, target, force = false) {
    ...
    try {
      ...
      if (page.style && page.style.background)
        target.style.backgroundImage = `url('${page.style.background}')`;
  ...
  unload(target) {
    ...
    if (this.loaded.indexOf(name) > -1)
      this.loaded.splice(this.loaded.indexOf(name), 1);
  }
  async updateStrollnav(page) {
    ...
    this.view.nav.innerHTML =
      '<table class="strollnav">' +
      `<tr><td class="action left">${hasdir.left ? "&larr;" : ""} ${
        navtext.left
      }</td>` +
      `<td class="action up">${hasdir.up ? "&uarr;" : ""} ${navtext.up}</td>` +
      `<td class="action middle"><b>${page.hname}</b></td>` +
      `<td class="action down">${hasdir.down ? "&darr;" : ""} ${
        navtext.down
      }</td>` +
      `<td class="action right">${hasdir.right ? "&rarr;" : ""} ${
        navtext.right
      }</td></tr>` +
      "</table>";
    ...
    if (page.nav && page.nav.down)
      navdown.onclick = () => this.pageMove("down");
    else navdown.onclick = undefined;
    if (page.nav && page.nav.left)
      navleft.onclick = () => this.pageMove("left");
    else navleft.onclick = undefined;
    if (page.nav && page.nav.right)
      navright.onclick = () => this.pageMove("right");
  ...
  handleSwipeMove(e) {
    ...
    const displacement = {
      x: pos.pageX - this.swipeStart.x,
      y: pos.pageY - this.swipeStart.y,
    };
    ...
    this.displacement.x = Math.max(
      Math.min(window.innerWidth, this.displacement.x),
      -window.innerWidth
    );
    this.displacement.y = Math.max(
      Math.min(window.innerHeight, this.displacement.y),
      -window.innerHeight
    );
